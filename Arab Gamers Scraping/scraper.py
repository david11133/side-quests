...
def main():
    ...
    # Create fieldnames with all filters
    base_fieldnames = ['ID', 'Type', 'SKU', 'Name', 'Published', 'Is featured?', 
                      'Visibility in catalog', 'Categories', 'Images', 
                      'Meta: _wp_page_template', 'product-description', 
                      'features', 'specification', 'reference URL', 'price', 'special price']
    
    filter_fieldnames = sorted(list(all_filter_names))
    fieldnames = base_fieldnames + filter_fieldnames
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
            ...
            for future in tqdm(concurrent.futures.as_completed(future_to_prod), total=len(all_products_to_scrape)):
                try:
                    ...
                    if details:
                        # Add filter data from our mapping
                        product_filters = global_filter_map.get(prod['url'], {})
                        for filter_name in filter_fieldnames:
                            details[filter_name] = product_filters.get(filter_name, '')
